...
**These rules are ABSOLUTE. Violation invalidates the entire evolution session.**

| Rule | Consequence of Violation |
|------|--------------------------|
| Must collect learning from ALL rounds before analysis | Patterns based on incomplete data are unreliable |
| Must cross-reference with historical ledger | Isolated patterns without context are misleading |
| Must classify updates as Minor or Major correctly | Minor updates bypass approval; misclassification is dangerous |
| Must NEVER apply Major updates without user approval | Unauthorized methodology changes |
| Must maintain version history (backup before changes) | No rollback capability if update causes problems |
| Must log ALL changes to ledger | Evolution history lost, patterns untrackable |

---

## 🔒 MANDATORY EXECUTION SEQUENCE

**You MUST follow this exact sequence. Skipping phases invalidates the evolution.**

### PHASE 1: Learning Collection
- [ ] Collect judge's overall learning brief from each round
- [ ] Collect individual learning briefs for each expert
- [ ] Collect critique logs from each critic
- [ ] Collect winner declarations with rationale
- [ ] Collect any user overrides with reasoning

**GATE CHECK:** Cannot analyze patterns without complete learning collection.

### PHASE 2: Pattern Detection
- [ ] Cross-reference with historical ledger
- [ ] Identify winning patterns (strengths to reinforce)
- [ ] Identify losing patterns (weaknesses to address)
- [ ] Identify context patterns (situational guidance)
- [ ] Verify pattern threshold met (2+ occurrences in competition, 2+ across arenas)

**GATE CHECK:** Must have verified patterns before proposing updates.

### PHASE 3: Update Classification
- [ ] Classify each update candidate as Minor or Major
- [ ] Minor: clarifications, examples, reinforcements (threshold: 1 occurrence)
- [ ] Major: new frameworks, structural changes (threshold: 3 occurrences)
- [ ] Document classification rationale for each

**GATE CHECK:** All updates must be classified before applying any.

### PHASE 4: Update Application
- [ ] Create backup of current skill version
- [ ] Apply Minor updates automatically
- [ ] Present Major updates for user approval
- [ ] Wait for approval before applying Major updates
- [ ] Log all changes to evolution log

**GATE CHECK:** Backups must exist before any changes applied.

### PHASE 5: Record Keeping
- [ ] Update win records for each expert
- [ ] Update skill version history
- [ ] Log all changes to ledger
- [ ] Check spawn condition (Synthesis won all rounds?)

**GATE CHECK:** All records must be updated to complete evolution session.
...
## 📋 SKILL EVOLUTION COMPLIANCE SUMMARY

### Signature Element Verification

| Signature Element | Found | Evidence Location |
|-------------------|-------|-------------------|
| Learning collected from ALL rounds | ☐ | |
| Historical ledger cross-referenced | ☐ | |
| Updates classified (Minor/Major) | ☐ | |
| Backups created before changes | ☐ | |
| Major updates awaiting approval (not auto-applied) | ☐ | |
| All changes logged to ledger | ☐ | |

**Evolution Validity:** [VALID if all checked / INVALID if any unchecked]

---

*webinar-skill-evolver v2.0.0*
