...
- Core Wound alignment (see below)

**Core Wound Alignment Check (v1.1 addition):**

Core Wounds (from Parris Lampropoulos's Schema Therapy teaching) are childhood emotional injuries that drive adult purchasing behavior. They operate BENEATH surface-level emotional language.

**Reference:** References/CORE-WOUNDS-FRAMEWORK.md

**When evaluating contextual appropriateness, check:**
- Does the copy touch the dominant Core Wound(s) identified in upstream research?
- Does the emotional arc IMPLICITLY address the wound without naming it?
- Does blame externalization align with wound patterns? (e.g., Failure wound → "not your fault" framing)

**CRITICAL:** Core Wound alignment is INFORMATIONAL, not gating. Per Parris's methodology: "Learn it, internalize it, then forget it and write naturally." This check identifies enhancement opportunities — copy that misses wound alignment still passes if other criteria are met. Add wound alignment observations to improvement suggestions.
...
### Layer 2.5: Arena Revision Generation

**Purpose:** Generate multiple REVISION CANDIDATES for high-priority issues through 6 legendary copywriter personas, then score and rank for human selection.
...
```
IDLE --> TRIGGERED --> LAYER_0 --> HUMAN_CHECKPOINT --> LAYER_1 --> LAYER_2 --> ARENA --> LAYER_3 --> LAYER_4 --> LAYER_5 --> COMPLETE
                           ↓                                 ↓          ↓          ↓          ↓          ↓          ↓
                        [GATE_0]                          [GATE_1]   [GATE_2]  [GATE_2.5] [GATE_3]   [GATE_4]   [GATE_5]
                           ↓                                 ↓          ↓          ↓          ↓          ↓          ↓
                        PASS/FAIL                         PASS/FAIL  PASS/FAIL  HUMAN_SEL PASS/FAIL  PASS/FAIL  PASS/FAIL
```

#### Persona Panel (6 Experts)

| Persona | Revision Lens |
|---------|--------------|
| **Makepeace** | Flow-focused fix — restructure for momentum, add bridges where stalled |
| **Halbert** | Entertainment-focused fix — inject life, story, drama, personality |
| **Schwartz** | Sophistication-calibrated fix — recalibrate claims, proof depth, language |
| **Ogilvy** | Credibility/clarity-focused fix — precision surgery, replace vague with specific |
| **Clemens** | Simplification-focused fix — complex ideas into 12-year-old language, binary reframes |
| **Bencivenga** | Proof-upgrade-focused fix — strengthen evidence, add institutional backing |

#### 7 Judging Criteria

| Criterion | Weight | What It Measures |
|-----------|--------|------------------|
| **Issue Resolution Impact** | 20% | Does it actually fix the identified problem? — CRITICAL |
| **Voice Preservation** | 20% | Does it maintain established voice and tone? Voice breaks destroy credibility |
| **Flow Enhancement** | 15% | Does it improve or maintain momentum and transitions? |
| **Clarity Improvement** | 15% | Does it make the passage clearer without dumbing down? |
| **Slop Elimination** | 10% | Does it remove AI telltales and weak language? |
| **Brevity** | 10% | Does it say the same or more with fewer words? |
| **Threading Preservation** | 10% | Does it maintain mechanism name, root cause anchor, callbacks? |

#### Execution Flow

1. **Issue Prioritization** — Identify P1/P2 issues from Layer 2 critique
2. **Multi-Perspective Revision Generation** — Each persona generates fix approach for each issue
3. **Judging Round** — Score all revisions against 7 criteria (weighted)
4. **Ranking & Rationale** — Rank top 5 with explicit reasoning per issue
5. **Human Selection Checkpoint** — Present ranked revisions; human selects or requests regeneration

**GATE_2.5:** BLOCKING — Requires human selection for each prioritized issue before Layer 3 proceeds.
...
```json
{
  ...
  "quality_scores": {
    ...
    "core_wound_alignment": {
      "dominant_wounds_from_upstream": ["[wound1]", "[wound2]"],
      "wound_touches_detected": "[count of copy sections that touch dominant wounds]",
      "alignment_observations": "[how copy aligns or misses wound patterns]",
      "enhancement_suggestions": "[optional: how copy could deepen wound resonance]"
    }
  },
...
|---------|------|---------|
| 1.4 | 2026-02-12 | Model Assignment Table: Added Binding Model Assignment Table. Haiku for infrastructure (Pre only), opus for all analytical/creative/validation layers (0-4), sonnet for final delivery packaging (5). Editorial is uniquely opus-heavy — even Layer 0 requires full-document analytical read. |
| 1.3 | 2026-02-04 | CORE WOUND INTEGRATION: Added Core Wound alignment check to Tier 3 (Contextual Appropriateness) — informational check per Parris methodology ("learn it, internalize it, forget it"). Added core_wound_alignment section to editorial-package.json output schema (dominant wounds from upstream, wound touches detected, alignment observations, enhancement suggestions). Reference: Skills/foundation/00-core-wounds/CORE-WOUNDS-FRAMEWORK.md |
| 1.2 | 2026-02-03 | Added Layer 2.5 Arena integration with 6-persona revision generation panel, 7 judging criteria (Issue Resolution Impact 20%, Voice Preservation 20%, Flow Enhancement 15%, Clarity Improvement 15%, Slop Elimination 10%, Brevity 10%, Threading Preservation 10%), BLOCKING human selection checkpoint at Gate 2.5, state machine diagram, critique-to-revision mapping. |
...
**Skill Status:** COMPLETE (EXCELLENT) — All 29 microskills built across 6 layers + Layer 2.5 Arena integration, all 7 guardrails implemented.
