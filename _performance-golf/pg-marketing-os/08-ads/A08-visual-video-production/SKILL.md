...
This skill requires Gemini media tools (image generation, video generation, image editing) and ElevenLabs audio tools (TTS, music, SFX). At Layer 0, call:
- `ToolSearch("gemini image")` and `ToolSearch("gemini video")` for visual/video generation
- `ToolSearch("elevenlabs")` for audio production
