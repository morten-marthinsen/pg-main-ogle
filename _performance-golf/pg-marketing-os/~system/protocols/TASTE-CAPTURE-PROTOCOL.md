...
**Version:** 1.0
**Created:** 2026-02-15
**Purpose:** Capture, structure, and operationalize the human reviewer's revision patterns as reusable taste constraints
...
The human reviewer's edits are the richest taste signal in the system. Every time they revise Arena output or editorial drafts, they're encoding preferences that should persist across projects. But currently:
...
Every human revision is captured as a structured taste edit entry.
...
AFTER EVERY HUMAN EDIT SESSION:
  1. COMPARE original output with the human reviewer's revised version
  ...
  3. For EACH change:
     ...
     c. Write reason (ask the human reviewer if unclear)
     d. Assess generalizability
     e. Map to soul_dimension
  4. WRITE all taste_edit entries to:
     [project]/TASTE-EDITS.yaml
  5. LOG taste_revision_entry to learning-log (see SCHEMA.md v2.0)
```
...
| **~system/SYSTEM-CORE.md** | Cross-references this protocol; mandates capture after every edit session |
...
```
The human reviewer edits Arena output or editorial draft
  │
  ├── Claude captures each edit as taste_edit entry
  ├── Writes to [project]/TASTE-EDITS.yaml
  ├── Logs taste_revision_entry to learning-log
  │
  ▼
