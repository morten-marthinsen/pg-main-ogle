...
> **Dependencies:** PDP-01-ecomm-strategist
...
- Feature package ready for PDP-03 (Hero), PDP-07 (Section Copy), PDP-08 (Micro-Scripts)
...
**Upstream:** Receives `ecomm-strategy.yaml` from PDP-01, `campaign-brief-package.json` from Skill 09
**Downstream:** Feeds `feature-package.json` to PDP-03 (Hero), PDP-07 (Section Copy), PDP-08 (Micro-Scripts)
...
1. `04-page-builder/skills/PDP-02-feature-naming/ARENA-LAYER.md` — skill-specific criteria and weights
...
**Specification File:** `PDP-02-ARENA-LAYER.md`
...
```json
{
  ...
  "skill_id": "PDP-02-feature-naming",
...
| Failure | Remediation |
|---------|-------------|
| Strategy missing feature research scope | HALT -- return to PDP-01 |
...
14. **Feature names must be used consistently downstream** -- PDP-03, PDP-07, PDP-08 must use exact names.
15. **Hero feature must appear in hero section** -- PDP-03 headline/subhead references hero feature.
16. **Feature hierarchy must match strategy priority** -- PDP-01 priority informs feature prominence.
...
|------|-----------------|-------------|
| Gate 0 | Strategy missing | HALT -> Request PDP-01 execution |
| Gate 0 | Feature scope empty | HALT -> Return to PDP-01 for scope definition |
