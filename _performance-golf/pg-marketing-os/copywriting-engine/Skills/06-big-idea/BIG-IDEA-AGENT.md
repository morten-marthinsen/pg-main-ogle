...
**Version:** 4.3 (RSF Complete + Schema Distance Framework + Transformation Operators)
**Skill:** 06-big-idea
**Position:** Phase 1, Step 6 (Research & Strategy)
**Type:** RSF-Driven Synthesis + Creative Generation
**Dependencies:** 03-root-cause, 04-mechanism, 05-promise outputs
**RSF Dependencies (Required):** expectation_schema.json, latent_resonance_field.json (including FSSIT candidates)
...
**Success Criteria (v4.0 RSF-Enhanced):**
- RSF inputs loaded and validated
- ≥9 Big Idea candidates generated (3 strategies × 3 variants minimum)
- FSSIT-first generation protocol followed for all candidates
- Schema distance gating applied (all candidates SD 4-8 or flagged)
- Tournament-style selection executed for final ranking
- All candidates include ≥10 headlines and ≥3 leads
- Coherence validation passes (all components align)
- Top candidate RSF composite score ≥ 7.5
- Human checkpoint with ≥5 candidates presented
- Handoffs complete for downstream copy skills

**Critical Architecture (v4.0):** This skill is RSF-DRIVEN. Every generation decision is informed by expectation schema (what to violate) and latent resonance field (what to connect to). FSSIT candidates are the STARTING POINT, not a validation check.
...
## Microskill Layers (Refactored for Synthesis)

| Layer | Purpose | Document |
|-------|---------|----------|
| **Layer 0** | Input Loading + Coherence Check | [L0-INPUT-SYNTHESIS.md](skills/layer-0/L0-INPUT-SYNTHESIS.md) |
| **Layer 1** | Big Idea Type Selection + Strategy | [L1-TYPE-STRATEGY.md](skills/layer-1/L1-TYPE-STRATEGY.md) |
| **Layer 2** | Candidate Generation (9+) | [L2-CANDIDATE-GENERATION.md](skills/layer-2/L2-CANDIDATE-GENERATION.md) |
...
| **Layer 3** | Validation + Scoring | [L3-VALIDATION-SCORING.md](skills/layer-3/L3-VALIDATION-SCORING.md) |
...
│  Generate 5+ Big Idea candidates using selected types                        │
│  For each: Synthesize RC + Mech + Promise with creative wrapper              │
│  Generate 10+ headlines per candidate                                        │
│  Write 3+ full leads per candidate                                           │
└────────────────────────────────┬────────────────────────────────────────────┘
                                 ↓
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Layer 3: Validation + Scoring                             │
│  Score each candidate (novelty, coherence, impact, differentiation)          │
...
### RSF Input Constraints (v4.0 — CRITICAL)
- NEVER proceed without expectation_schema.json loaded — RSF requires schema awareness
- NEVER proceed without latent_resonance_field.json loaded — RSF requires FSSIT candidates
...
### Multi-Variant Generation Constraints (v4.0 — MANDATORY)
- MUST generate minimum 9 candidates (3 strategies × 3 variants)
...
- NEVER produce fewer than 9 candidates — This is a structural quality gate
...
### Generation Constraints (Layer 2)
- NEVER produce fewer than 9 Big Idea candidates (increased from 5 in v4.0)
- NEVER produce Big Ideas without headlines — Minimum 10 headlines per candidate
- NEVER produce Big Ideas without leads — Minimum 3 full leads (200-500 words each) per candidate
- NEVER produce headline without vault_inspiration reference
- NEVER produce lead without explicit lead_type classification
- NEVER omit proof_architecture from any candidate
...
**VERSION 4.0** — Complete RSF-Driven Overhaul per system-wide RSF integration initiative.
...
**Layers (Updated for RSF):**
- [x] Layer 0: Input Synthesis + RSF Loading + Coherence Check
- [x] Layer 1: FSSIT Mapping + Type Strategy + Schema Distance Planning
- [x] Layer 2: Multi-Variant Candidate Generation (9+ candidates)
- [x] Layer 3: Pre-flight Filter + Tournament Selection + RSF Scoring
