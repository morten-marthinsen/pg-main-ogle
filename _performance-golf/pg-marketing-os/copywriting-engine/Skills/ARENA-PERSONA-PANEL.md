...
**Created:** 2026-02-03
**Updated:** 2026-02-05
**Purpose:** Defines the 7 competitors + 1 adversarial critic used across all Arena-based skill layers

---

## THE PANEL

The Arena system uses **7 competitors** (6 legendary copywriters + The Architect) and **1 dedicated adversarial critic**. Each competitor generates candidates based on their documented approach. The Critic identifies weaknesses. Competitors revise. Then candidates are judged against skill-specific criteria. 3 rounds mandatory.

**Reference:** See `ARENA-CORE-PROTOCOL.md` for full execution protocol (3-round competition, critique-revise, learning briefs, context management).
...
- Full protocol in `/CopywritingEngine/SYNTHESIZER-LAYER.md`
...
**IMPORTANT:** The full 3-round execution protocol, including critique-revise phases, learning briefs, context compression, and MC-CHECK schedule, is defined in `ARENA-CORE-PROTOCOL.md`. This section provides the summary.
...
**Why This Exists:** When running in Agent Team mode (see `ARENA-CORE-PROTOCOL.md` v2.0), each persona runs as a **separate Claude instance** with its own 200K context window. Each persona agent needs a **self-contained prompt package** — everything it needs to generate without access to other personas' context.
...
persona_agent_package:
  ...
  specimens: "[type-matched patterns from skill's 0.2.6-curated-gold-specimens.md]"
  ...
  forbidden_behaviors: "[from ARENA-CORE-PROTOCOL.md]"
  voice_preservation: |
...
judge_agent_package:
  ...
  outputs:
    ...
    - ranking: "[1-7]"
    - learning_brief: "[per Learning Brief spec in ARENA-CORE-PROTOCOL.md]"
```
...
See `/CopywritingEngine/SYNTHESIZER-LAYER.md` for complete synthesis protocol including:
