...
**Anti-Degradation:** Read in conjunction with [[RESEARCH-ANTI-DEGRADATION.md]] (12 structural fixes)
...
GATE_1_CHECKLIST:
  ...
  GATE_STATUS:
    IF any status = "FAIL" → GATE CLOSED
...
**When a gate fails, 3 MANDATORY expansion rounds are required before human escalation.**

### Gate Failure Response (v1.1 — aligned with RESEARCH-ANTI-DEGRADATION.md Fix 10)

```
GATE FAILED → DO NOT proceed. DO NOT invent new gate statuses.
Gate status can ONLY be PASS or FAIL. No "PARTIAL_PASS", "conditional pass", "WARNING", or any other value.

EXPANSION ROUND 1:
  1. IDENTIFY which buckets failed (from QUOTE VOLUME CHECK table)
...
EXPANSION ROUND 2:
  ...
  8. IF PASS → proceed. IF FAIL → ROUND 3.

EXPANSION ROUND 3:
  1. IDENTIFY REMAINING deficit buckets
  2. Use Perplexity deep research for underperforming buckets
  3. Try Apify specialized scrapers
  4. Scrape ANY remaining accessible sources
  5. Extract and tag new quotes
  6. UPDATE PROJECT-STATE.md with new counts
  7. Re-run QUOTE VOLUME CHECK
  8. IF PASS → proceed. IF FAIL → ESCALATE TO HUMAN.

HUMAN ESCALATION (only after ALL 3 rounds):
  Present exact counts vs targets, what was tried, why market may lack data.
  Options: (a) approve reduced threshold, (b) provide sources, (c) increase budget, (d) abandon.
```

### Structural Checkpoint File: GATE_1_VERIFIED.yaml

**Layer 2 CANNOT execute unless this file exists in `[project]/checkpoints/`:**

```yaml
# GATE_1_VERIFIED.yaml — ONLY create when ALL thresholds met
gate: GATE_1
status: PASS  # The ONLY acceptable value. No other status exists.
verified_counts:
  total: [exact count >= 1000]
  pain: [exact count >= 300]
  hope: [exact count >= 250]
  root_cause: [exact count >= 200]
  solutions_tried: [exact count >= 150]
  competitor_mechanism: [exact count >= 100]
  villain: [exact count >= 75]
expansion_rounds_completed: [0-3]
```

**If ANY count is below its minimum, this file MUST NOT be created.** Creating it with insufficient counts is a protocol violation.

### FORBIDDEN RATIONALIZATIONS (IMMEDIATE HALT)

If any of these phrases appear in gate reasoning, the gate check is INVALID:

| Rationalization | Why It's Forbidden |
|-----------------|-------------------|
| "quality over quantity" | BOTH required. Quality filtering happens after quantity met. |
| "partial pass" / "PARTIAL_PASS" | Does not exist. Gates are PASS or FAIL only. |
| "conditional pass" | Does not exist. Gates are PASS or FAIL only. |
| "sufficient for analysis" | Thresholds are non-negotiable. |
| "close enough" / "approximately" | Numbers are exact, not approximate. |
| "quality is HIGH so we can proceed" | Quality does not compensate for missing quantity. |
| "proceed with what we have" | Expansion loop is mandatory. |
| "the research is comprehensive" | Comprehensiveness is measured by counts vs thresholds. |

**Example (CORRECT):**
```
GATE 1 FAILED: Pain quotes = 234 (required 300)
STATUS: FAIL
ACTION: Begin Expansion Round 1 — targeting Pain bucket deficit of 66 quotes
```
...
**Every subagent spawn MUST be logged:**

```yaml
subagent_spawn_log:
  skill: "[e.g., 1.4-C Reddit Scraper]"
  timestamp: "[ISO 8601]"
  compliance:
    model_used: "[haiku | sonnet | opus]"
    model_matches_table: [true | false]  # Must be true
    persona_included: [true | false]  # Must be true
    persona_name: "[e.g., PERSONA_SCRAPER]"
    quote_targets_included: [true | false]  # Must be true for Layer 1
    all_8_sections_present: [true | false]  # Must be true
  override:
    model_overridden: [true | false]
    override_reason: "[only if true — requires human approval]"

Save to: projects/[project]/validation-logs/spawn_log.yaml (append)
```

**IF model_matches_table = false AND no human approval → PROTOCOL VIOLATION.**
**IF all_8_sections_present = false → STRUCTURAL BLOCK. Do not spawn.**
