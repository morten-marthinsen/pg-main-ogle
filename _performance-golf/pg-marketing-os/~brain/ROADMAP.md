...
### 2.0 Context Budget Engineering
**Priority:** HIGH
**Status:** Phase 1 COMPLETE — Audit tool built and initial results generated

- **Audit tool:** `scripts/audit_context_load.py` — maps per-skill context load (file sizes → token estimates)
- **Audit results:** `CONTEXT-BUDGET-AUDIT.md` — full matrix of skill × system file loads with zone classifications
- **Key findings:** Research skill (01) at 632K tokens (RED zone), Story (12) at 294K (ORANGE), Big Idea (06) at 203K (ORANGE). CLAUDE-CORE.md adds ~6.5K tokens overhead per skill. CLAUDE-SKILL-INDEX.md adds ~6.9K but only one section is needed per skill.
- **Next:** Context Compression Pilot (1.1B) — test compression on highest-load skills
- **Full plan:** `CONTEXT-COMPRESSION-PILOT.md`

### 2.1 Comprehensive Upgrade Plan — Implementation Status
**Priority:** HIGH
**Status:** Phases 1-7 IMPLEMENTED (March 7, 2026)

All 23 upgrades from the Comprehensive Upgrade Plan v3 have been implemented across 7 phases:

| Phase | Upgrades | Status |
|-------|----------|--------|
| **Phase 1: Foundation** | 1.2 Programmatic Gates, 1.7 Task Triage, 2.5 Proportionality, 2.1 Active Recitation | COMPLETE |
| **Phase 2: Context Optimization** | 1.1A Audit, 1.1B Pilot Plan, 2.7 Context Zones | COMPLETE |
| **Phase 3: Verification Architecture** | 1.3 Scoped Verification, 1.4 Foundation Integrity, 1.5 Prose QV, 2.4 Editorial Isolation | COMPLETE |
| **Phase 4: Arena Improvements** | 2.2 Convergence Governor, 2.3 Controlled Diversity | COMPLETE |
| **Phase 5: Learning + Self-Improvement** | 1.6 Self-Learning Loop, 3.1 Autoresearch, 3.5 Meta-Prompt | COMPLETE |
| **Phase 6: Context Budget Rollout** | 1.1C Rollout Plan | PLAN COMPLETE (awaiting pilot results) |
| **Phase 7: Hardening** | 2.6 File Integrity, 2.8 Uncertainty, 3.2 Pre-Mortem, 3.3 Injection Guard, 3.4 Constraint Ledger, 3.6 Arena Hardening | COMPLETE |

**Source:** `Marketing-OS — Comprehensive Upgrade Plan v3.md`
**PARKED:** Autoresearch automated scoring, Structured Reasoning Selection, Session Architecture for 1M
