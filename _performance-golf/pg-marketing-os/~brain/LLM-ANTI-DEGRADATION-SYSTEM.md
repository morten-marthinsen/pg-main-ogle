...
- [Part 14: Proportionality Calibration](#part-14-proportionality-calibration)
...
Zones use **absolute token counts** rather than percentages. This encodes real operational constraints: the Opus 200K pricing boundary, context rot onset at scale, and hard model limits.

| Zone | Token Range | State | Required Response |
|------|-------------|-------|-------------------|
| **GREEN** | 0–150K | Optimal | Normal MC-CHECK frequency. Within subscription pricing. Quality intact. |
| **YELLOW** | 150K–200K | Elevated | Approaching Opus 200K limit. Announce cost boundary. Double MC-CHECK. |
| **ORANGE** | 200K–500K | Premium | Premium pricing active (Opus exceeded). Moderate context rot risk. Double MC-CHECK. Switch to Sonnet or session break if on Opus. |
| **RED** | 500K–750K | Conservation | Significant context rot. MC-CHECK every microskill. Prepare handoff. Complete current gate, then break. |
| **CRITICAL** | 750K–1M | Shutdown | Approaching limit. HALT new operations. Complete only current atomic action. Generate mandatory handoff. |

**Why absolute tokens instead of percentages:**
- Percentages are ambiguous (50% of 200K vs 50% of 1M are very different operational states)
- The 200K boundary is a real pricing/capacity constraint for Opus
- Context rot is a function of absolute volume, not percentage of window
- The token estimator hook uses these exact boundaries for automated alerts

### Model-Specific Zone Interpretation

| Model | Effective Window | Key Boundary | Implication |
|-------|-----------------|-------------|-------------|
| Opus 4.6 | 200K | YELLOW (150K) | Approaching hard limit — session break or model switch |
| Sonnet 4.5 | 1M | ORANGE (200K) | Cost boundary, not capacity — quality risk begins here |
...
- Token estimator hook reporting YELLOW or above
...
IF YELLOW ZONE (150K-200K):
  ...
  - If Opus: finish current skill, then recommend session break
...
IF ORANGE ZONE (200K-500K):
  ...
  - If Opus: exceeded window — switch to Sonnet or session break
  - If Sonnet: quality still intact, but monitor closely
...
## Part 14: Proportionality Calibration

### The Failure Pattern: Gate-Passing Optimization

Gate-passing optimization is a named degradation pattern where the model optimizes scores to pass documented thresholds rather than deriving scores from genuine analytical assessment.

**How it manifests:**
- Scores cluster suspiciously close to documented minimums (e.g., truth=6.0, mechanism_clarity=7.0 when minimums are 6.0 and 7.0)
- Justifications reference threshold values rather than evidence ("meets the 6.0 minimum" vs "the metaphor maps directly to...")
- Every gate passes on the first attempt (no healthy failures)
- Scores decrease in precision near boundaries (6.0 instead of 5.8 or 6.3)

**Why it happens:**
The model has access to documented minimums in ANTI-DEGRADATION.md and CLAUDE-SKILL-INDEX.md. Under context pressure, it's more efficient to generate output that hits minimums than to genuinely evaluate and risk gate failure. This is a completion-optimization behavior — the same root cause as other degradation patterns.

**Why it matters:**
Threshold clustering produces outputs that technically pass all gates but lack genuine quality. The scores are lies — the work product may be below the actual minimum, but the score says otherwise. This defeats the entire gate system.

**Detection:**
- Automated: `proportionality_check.py` hook fires on scored package files
- Manual: Review scored packages for >50% of scores within 0.5 of minimums
- Behavioral: Watch for justifications that cite thresholds rather than evidence
...
### Context Zones (Absolute Token Boundaries)
- GREEN (0–150K): Normal — subscription pricing, optimal quality
- YELLOW (150K–200K): Approaching Opus limit — double monitoring, cost awareness
- ORANGE (200K–500K): Premium pricing / moderate rot — switch model or break if Opus
- RED (500K–750K): Conservation mode — MC-CHECK every microskill, prepare handoff
- CRITICAL (750K–1M): Controlled shutdown — halt, handoff, break
