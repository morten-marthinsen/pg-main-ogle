...
**Dependencies:** 01-research outputs, 02-proof-inventory outputs, SOUL.md (project-level)
...
## Pre-Execution Requirements (v4.0)

**BEFORE any root cause execution, complete these steps:**

1. **READ** `ROOT-CAUSE-ANTI-DEGRADATION.md` v2.0 — mandatory
2. **READ** `SOUL-MD-PROTOCOL.md` — understand Soul.md integration
3. **LOAD** `[project]/SOUL.md` — if exists, this constrains all expression decisions
4. **CREATE** project infrastructure:
   - `[project]/03-root-cause/PROJECT-STATE.md`
   - `[project]/03-root-cause/PROGRESS-LOG.md`
   - `[project]/03-root-cause/checkpoints/` directory
5. **VALIDATE** all upstream inputs exist (research FINAL_HANDOFF, proof FINAL_HANDOFF, brief)
6. **DELETE** any stale artifacts from previous failed attempts
7. **ONLY THEN** begin Layer 0
...
## Three-Phase Architecture (v4.0 — Concept/Naming Separation)

```
PHASE A: CONCEPT DISCOVERY (Layer 1) — "What IS the root cause?"
┌─────────────────────────────────────────────────────────────────┐
│ FIND the root cause from research data. Plain language.          │
│ Output: 3-5 root cause CONCEPTS described in plain language.    │
│                                                                 │
...
│ 1.7 Concept Synthesis            ← Produce 3-5 ranked concepts  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
              ╔═══════════════════════════════════╗
              ║   HUMAN CHECKPOINT (BLOCKING)     ║
              ║                                   ║
              ║   Present 3-5 root cause concepts ║
              ║   in plain language with:         ║
              ║   - What they think (surface)     ║
              ║   - What's really going on (root) ║
              ║   - Why nothing has worked         ║
              ║   - Ranking + reasoning            ║
              ║                                   ║
              ║   Anthony SELECTS the concept.    ║
              ║   NO naming. NO expression method.║
              ║   Just: "THIS is the right root   ║
              ║   cause to build the campaign on." ║
              ║                                   ║
              ║   GATE: CONCEPT_APPROVED.yaml     ║
              ╚═══════════════════════════════════╝
                              ↓
PHASE B: EXPRESSION & NAMING (Layer 2) — "How do we FRAME it?"
┌─────────────────────────────────────────────────────────────────┐
│ Take the APPROVED concept and generate naming/framing variants. │
│ Soul.md constrains all expression decisions.                    │
│                                                                 │
...
│ 2.7 Expression Synthesis         ← 3+ expression variants       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
PHASE C: ARENA + VALIDATION (Layer 2.5 + Layer 3) — existing protocol
```

### Why Concept/Naming Separation Matters (v4.0)

**The old failure mode:** AI tries to simultaneously identify the root cause AND name/frame it. A good concept gets a bad name. A mediocre concept gets a great name and wins Arena. The CONCEPT selection and the NAMING selection are conflated.

**The fix:** Two separate human checkpoints.
1. **Concept checkpoint:** "Is THIS the right root cause?" (plain language, no creative framing)
2. **Naming checkpoint (Arena):** "Which EXPRESSION of this concept is strongest?" (creative framing of the approved concept)

**What changes:**
- Layer 1 output is now 3-5 plain-language concept descriptions, NOT named/framed root causes
- Human approval happens BEFORE any naming or expression method selection
- Layer 2 only runs on the APPROVED concept — never on rejected concepts
- Arena (Layer 2.5) evaluates EXPRESSIONS of the same concept, not competing concepts

### Human Checkpoint Format (Concept Approval)

Present to Anthony in this format:

```markdown
## Root Cause Concept Candidates

### Concept A: [Plain-language 1-sentence description]
**What they think:** [Surface understanding in audience language]
**What's really going on:** [Root cause in plain language — no naming, no framing tricks]
**Why nothing has worked:** [Plain explanation of past failure connection]
**Mechanism solvable:** Yes/No + brief note
**Proof pathway:** Strong/Medium/Weak + brief note
**Ranking rationale:** [Why this is ranked where it is]

### Concept B: [Plain-language 1-sentence description]
[Same format]

### Concept C: [Plain-language 1-sentence description]
[Same format]

---
**Recommended:** [Which concept and why]
**Awaiting your selection before proceeding to naming/expression.**
```
...
concept_approved:
  ...
  plain_statement: "[The root cause in plain language]"
  what_they_think: "[Surface problem]"
  what_real: "[Root cause]"
  why_nothing_worked: "[Past failure explanation]"
  anthony_notes: "[Any direction from Anthony about framing]"
  approved_at: "[timestamp]"
  total_concepts_presented: [number]
```

**HARD GATE:** Layer 2 (Expression) CANNOT begin without CONCEPT_APPROVED.yaml existing. This file is written ONLY after human selection.
...
|------------|---------|----------|
| 0.0.1 | Vertical Profile Loader | Load vertical config (persona panel, specimens, taste defaults, anti-slop) |
| 0.2.7 | `0.2.7-persona-voice-loader.md` | Load persona-specific voice specimens for System 2 Arena calibration |
| 0.2.8 | `0.2.8-tier1-expression-reference.md` | Load TIER1 conversion-validated expression patterns for expression anchoring |
...
| 2.8 | Expression Anchoring Score | [2.8-expression-anchoring-score.md](skills/layer-2/2.8-expression-anchoring-score.md) |
...
## Execution Flow (v4.0 — Concept/Naming Separation)

```
                    INPUT: Deep Research + Proof Inventory + Soul.md
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│                     PRE-EXECUTION (haiku)                                 │
│   Read anti-degradation → Load Soul.md → Create infrastructure           │
│   → Validate inputs                                                      │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│              PHASE A: CONCEPT DISCOVERY (Layer 1, opus)                   │
│                                                                           │
│   Research Patterns → Symptom Convergence → False Belief → Hidden Layer   │
│                                    │                                      │
│                          Mechanism Check → Proof Check                    │
│                                    │                                      │
│                    Concept Synthesis → 3-5 PLAIN-LANGUAGE Concepts        │
│                                    │                                      │
...
└───────────────────────────────────┬───────────────────────────────────────┘
                                    ...
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│              ★ HUMAN CHECKPOINT: CONCEPT APPROVAL (BLOCKING) ★            │
│                                                                           │
│   Present 3-5 concepts in plain language (NO naming, NO framing)          │
│   Anthony selects THE root cause concept to build the campaign on         │
│                                    │                                      │
│                    GATE: CONCEPT_APPROVED.yaml                           │
│              (Layer 2 CANNOT begin without this file)                    │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│         PHASE B: EXPRESSION & NAMING (Layer 2, opus)                      │
│         Takes APPROVED concept only. Soul.md constrains framing.          │
│                                                                           │
│   Simple Reframe ─┐                                                       │
│   Named Syndrome ─┼─→ Niche Matching → Expression Synthesis               │
│   Villain ────────┤     ≥3 expression variants of approved concept       │
│   Metaphor ───────┤                                                       │
│   Dual Problem ───┘                                                       │
│         +                                                                 │
│   Quote-First Generation (2.8):                                           │
│     Generate 3+ expressions derived directly from audience quotes.        │
│     Score ALL variants (concept-down + quote-first) against research      │
│     quotes, TIER1 patterns, and FSSIT candidates.                        │
│     Only variants scoring 6.5+ proceed to Arena.                         │
│                                    │                                      │
...
└───────────────────────────────────┬───────────────────────────────────────┘
                                    ...
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│                     LAYER 3: VALIDATION (opus)                            │
│                                                                           │
│         Truth → Alignment → Proof → Resonance → Synthesis                 │
│         All 10 critical rules validated                                   │
│         Soul.md compliance checked                                        │
│                                    │                                      │
│          DECISION: Approved / Revision / Blocked                         │
│                    GATE: LAYER_3_COMPLETE.yaml                           │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
...
│      root-cause-package.yaml + ROOT-CAUSE-SUMMARY.md + execution-log.md  │
│                                    │                                      │
...
STEP: Load RSF Data
  ...
  3. IF BOTH exist:
     ...
     - Extract schema_summary for framing reference
     - Extract resonance_summary for emotional anchoring
     - Log: "RSF data loaded — schema distance and resonance enhancement active"
  4. IF EITHER missing:
     - SET rsf_inputs_available: false
     - Log: "RSF data unavailable — proceeding with standard execution"
...
- LOAD identity tensions from FSSIT handoff (Section: Identity Tensions)
- USE identity tensions to inform villain construction — the villain EXPLOITS these tensions
- USE FSSIT candidates to inform root cause framing — which "false story" are we correcting?
- REFERENCE: `skills/protocols/FSSIT-HANDOFF-TEMPLATE.md` for field definitions

If `RSF_SKIP_ACKNOWLEDGED.yaml` exists:
- Note in execution log: "RSF skipped — root cause generated without identity tension data"
...
**Layer 1 → CONCEPT CHECKPOINT Gate (v4.0 — NEW):**
- AFTER completing all Layer 1 microskills (1.1-1.7)
- BEFORE starting any Layer 2 microskill
- VERIFY: ≥3 root cause concept candidates produced in plain language
- VERIFY: Each candidate has mechanism_check = PASS and proof_check = PASS
- PRESENT: All candidates to human for selection (see Human Checkpoint Format above)
- WAIT: Human selects concept. Write CONCEPT_APPROVED.yaml.
- **HARD GATE: CONCEPT_APPROVED.yaml MUST exist before Layer 2 begins**

**CONCEPT CHECKPOINT → Layer 2 Gate:**
- AFTER human approves concept (CONCEPT_APPROVED.yaml exists)
- BEFORE starting Layer 2 expression generation
- VERIFY: Selected concept loaded and confirmed
- LOAD: Soul.md constraints for expression decisions

### Quote-First Generation (v4.1 — Expression Anchoring)

In addition to concept-down expression methods (2.1-2.7), generate 3+ expressions derived directly from audience quotes. Extract the most emotionally loaded phrases from Pain and Solutions-Tried buckets and build expressions that reorganize those phrases into root cause framings. These quote-first candidates are scored alongside concept-down variants using the 3-dimension anchoring protocol. Only variants scoring 6.5+ proceed to Arena. See `skills/protocols/EXPRESSION-ANCHORING-PROTOCOL.md` for full scoring methodology.

**Layer 2 → Layer 2.5 Gate (MANDATORY):**
- AFTER completing Layer 2 expression anchoring score (2.8)
- BEFORE starting Arena Layer (2.5)
- VERIFY: Expression method selected, all three parts populated, ≥3 candidates with ≥3 variants each
...
- The Arena Layer is NOT optional. NOT deferrable. NOT "run separately." It executes between Layer 2 and Layer 3 EVERY TIME.
...
| 3.2 | 2026-02-12 | Infrastructure + Model Selection: Pre-Execution Requirements, model assignment binding table, stale artifact cleanup |
| 4.1 | 2026-02-23 | **Expression Anchoring Protocol.** Added 0.2.8 TIER1 Expression Reference loader (Layer 0) and 2.8 Expression Anchoring Score (Layer 2, after 2.7, before Arena). Quote-First Generation: 3+ expression variants derived bottom-up from audience quotes. All expressions scored on 3 dimensions (Quote Penetration 40%, TIER1 Pattern Match 30%, FSSIT Echo 30%). Only variants scoring 6.5+ proceed to Arena. Shared protocol: `Skills/protocols/EXPRESSION-ANCHORING-PROTOCOL.md`. |
| 4.0 | 2026-02-14 | **Concept/Naming Separation + Soul.md Integration.** Three-phase architecture: Phase A (concept discovery, plain language, 3-5 candidates) → Human Checkpoint (concept approval, CONCEPT_APPROVED.yaml) → Phase B (expression/naming of approved concept only). Soul.md loaded as mandatory taste constraint. Arena competitors constrained by Soul.md voice. Synthesizer uses Soul.md as guiding voice layer. |
