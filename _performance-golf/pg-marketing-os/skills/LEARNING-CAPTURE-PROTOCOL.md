...
- "Which skills consistently produce output Anthony is happy with?"
...
After every skill completion, prompt Anthony for a structured rating.
...
rating_entry:
  ...
  notes: |
    [Optional free-text — Anthony's specific feedback]

  edit_count: [integer]
    # Number of edits Anthony made (0 = no edits needed)
...
AFTER EVERY SKILL COMPLETION (Layer 4 output packaging):
  ...
  IF Anthony provides rating:
    → Log rating_entry to LearningLog/[skill]-learning.json
    → Include in skill's execution-log.md

  IF Anthony declines or skips:
...
Anthony's edit volume is a satisfaction signal. Many edits = the system missed his taste.
...
WHEN both explicit rating AND edit count are available:
  ...
  COMPARE:
    ...
    - IF large gap (e.g., 8 explicit but 25 edits):
        → Anthony may be generous with ratings
...
AFTER PROJECT COMPLETION (Skill 20 Editorial or client delivery):
  ...
  6. UPDATE: LearningLog/cross-skill-patterns.json with new patterns
```
...
AFTER EVERY SKILL COMPLETION (appended to Layer 4):
  ...
  STEP 1: LOG RUN ENTRY
    → Create/append run_entry to LearningLog/[skill]-learning.json
  ...
  STEP 2: CAPTURE TASTE REVISIONS (if applicable)
    → IF human edits were made during this skill:
        ...
        c. Log taste_revision_entry to LearningLog/[skill]-learning.json

  STEP 3: PROMPT FOR RATING
    → Present rating prompt to Anthony
    → Log rating_entry (or note if declined)

  STEP 4: LOG RATING ENTRY
    → Append rating_entry to LearningLog/[skill]-learning.json
...
| **CLAUDE.md** | Universal LEARN phase documented; mandatory after every skill |
...
2. ❌ Nagging Anthony for ratings if he declines
