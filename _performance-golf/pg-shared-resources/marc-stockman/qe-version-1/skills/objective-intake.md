...
description: Objective-to-Execution Orchestration Layer — outcome framing, persona selection, skill routing, execution mode classification
scope: Runs after session bootstrap, before Skill 0. Handles objective framing and persona selection.
trigger: Marc states a new objective
---
...
When Marc states what he wants, the system needs to answer four questions before Skill 0 optimizes the prompt:
...
4. **How big is this?** (Execution Mode)
...
Execution with full stack active
    ↓
Quality Engine (self audit, check)
```

**Exception — skip Objective Intake when:**
- Marc explicitly says "just do it" / "skip intake" / "execute directly"
- The objective is a standing command (`check`, `self audit`, `audit`, `initialize`)
...
| persona-ai-productivity-architect | Mac/tool setup, configuration, integration wiring, learning workflows, staying current on releases | Tool install/config, "how do I use X?", workflow design, "what's new with X?", Phase A-D signals | 9b4f51f1-c83a-4307-be6f-61be5045e6dd |
...
- `self-audit` — fires on "self audit" command
...
### Step 4: Execution Mode Classification
...
OBJECTIVE INTAKE (this skill)
  ...
  Step 4: Execution Mode → single-turn / multi-phase / recurring / exploratory?
    ↓
...
Execution (R-07 gate → Q1 → full stack)
    ↓
Quality Engine (self audit, check, audit)
```
...
This is v1.1 — the first deployed version. After 2-3 sessions of real use, review:
