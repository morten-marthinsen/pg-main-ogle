...
description: "Quality Engine Phases 4-6: Verification (CoVe), Adversarial Critique, Pre-Mortem (FMEA), Uncertainty Calibration, Convergence Governor, and Output Contract."
---
...
**DEFER TO `audit`** when Marc says "audit" (or "self audit" — same command). The audit skill is a standalone protocol Marc triggers after delivery: Step 0 orientation → loop Passes 1–4 until zero material changes → Pass 5 share → Pass 6 system learning. This skill's procedures (Skills 7-12) are the per-query pipeline that runs BEFORE delivery — they are the quality system that makes the first draft as good as possible so the audit finds fewer issues.

How they relate:
- **Audit Pass 1 (Verification)** ≈ This skill's **Skill 7** — but audit runs post-delivery as a check; Skill 7 runs pre-delivery as part of the pipeline
- **Audit Pass 2 (Adversarial)** ≈ This skill's **Skill 8** — same relationship
- **Audit Pass 3 (Pre-Mortem)** ≈ This skill's **Skill 9** — same relationship
- **Audit Pass 4 (Revise)** ≈ This skill's **Skill 11 + 12** convergence and quality gate

The audit is the second line of defense. This skill is the first.
...
*Quality Engine Phases 4-6 of 6. Receives from qe-reasoning-engine (Phases 2-3). Delivers final output to Marc. Post-delivery: audit and check-protocol available as second-line quality checks.*
