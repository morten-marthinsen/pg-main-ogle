...
description: "Skill 0 Prompt Optimization Engine. Classifies tasks, extracts true intent, builds self-contained problem statements with success criteria."
---

# Skill 0: Prompt Optimization Engine

**Version:** 1.4 | March 6, 2026
...
- The prompt is a standing command (`check`, `audit`, `initialize`)
...
run → Step 7: R-07 Gate → Q1 → Q2 → Execute → Q3 → L1 → L2 → Deliver (R-18 + R-19)
    ↓
Marc's choice: audit / check / next task
```
...
| `audit` | After delivery | Forensic audit with convergence loop — loops until clean |
