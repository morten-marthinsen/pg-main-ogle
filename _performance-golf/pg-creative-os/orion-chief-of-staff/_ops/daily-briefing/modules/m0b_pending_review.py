...
class PendingReviewModule(BriefingModule):
    ...
    def fetch_data(self) -> Any:
        ...
        # Pending items persist until explicitly triaged — NEVER auto-expire.
        # M9's freshness filter already prevents old transcripts from being
        # re-extracted. Once an item reaches status=="pending" in the KB, it
        # stays in Pending Review until Christopher approves, rejects, schedules,
        # or completes it. Skipping triage for a day (weekend, travel, etc.)
        # must not cause items to disappear.
        pending = all_pending
