# CI/CD MCP Test Client

This directory contains a simple Go-based test client for the `cicd-mcp-server`.
...
This client **requires** the `cicd-mcp-server` to be built and running in a separate terminal.

### 1. Run the Server

First, build and run the `cicd-mcp-server` from its root directory. It *must* be started with the `-http` flag.
