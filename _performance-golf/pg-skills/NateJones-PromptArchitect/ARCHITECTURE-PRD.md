...
### 5.6 The Multi-Agent Scaling Problem

> **Source:** MULTI-AGENT-SCALING-REFERENCE.md — Full article, research citations, and diagnostic prompts
> **Research Basis:** Google-MIT (December 2025), Cursor production systems, Gas Town (Steve Yegge), March 2025 multi-agent framework analysis (1,600+ execution traces)

**Core Finding:** Adding more agents to a system can make it perform worse — not diminishing returns, but actual degradation. The Google-MIT study documented configurations where more agents produced worse outcomes than fewer.

**Why:** Agents aren't GPUs. They're entities that need to coordinate, and coordination creates overhead that grows faster than capability. Past some threshold, most agents are effectively standing in line.

**The Serial Dependency Problem:** A serial dependency is any point where one agent's work blocks another's — waiting for a lock, checking shared state, coordinating on who handles what. At small scale (3-5 agents, one hour), the overhead is invisible. At large scale, it dominates. Enough serial dependencies and parallelism collapses: paying for a hundred agents, getting throughput of five.

**Key Metrics:**
- Once single-agent accuracy exceeds ~45%, adding agents yields diminishing or negative returns
- Tool-heavy multi-agent systems required 1.6-6.2x token budgets to match single-agent performance
- 44% of multi-agent breakdowns are system design problems + 32% inter-agent misalignment = 76% are design failures, not code failures
- Gartner predicts >40% of agentic AI projects canceled by end of 2027

**Convergence Evidence:** Teams that scaled past prototype phase — Cursor (hundreds of agents, week-long autonomous coding) and Gas Town (20-30 agents simultaneously) — solved the same problem independently and landed on the same counterintuitive patterns. The industry consensus (team collaboration, shared context, continuous operation) breaks at scale. What works looks too simple until you understand why simplicity is the point.

### 5.7 Six Rules for Multi-Agent Scaling

These rules eliminate serial dependencies. They are what the teams running hundreds of agents actually use — arrived at through failure, not philosophy.

**Rule 1: Two Tiers, Not Teams**

The consensus says agents should collaborate like human teams. Cursor tested this: agents with equal status, coordinating through shared file, claiming tasks, updating status. Twenty agents slowed to effective throughput of two or three. With no hierarchy, agents became risk-averse — hard problems sat unclaimed because claiming meant responsibility for failure.

**The rule:** Strict two-tier hierarchy. Planners create tasks, workers execute them, a judge evaluates results. Workers don't coordinate with each other — they don't even know other workers exist. Each picks up a task, executes in isolation, pushes changes, terminates.

- Flat systems maximize serial dependencies
- Deep hierarchies (3+ levels) accumulate drift as objectives mutate through delegation layers
- Two tiers is the minimum structure that enables coordination while preserving parallelism

**Rule 2: Workers Stay Ignorant**

Workers perform better when deliberately kept ignorant of the big picture. When Cursor's workers understood broader context, they experienced scope creep — deciding adjacent tasks needed doing, reinterpreting assignments, conflicting with other workers.

**The rule:** Minimum viable context. Workers receive exactly enough to complete their assigned task, no more. Enforce through information hiding, not instructions workers might override. A worker that only knows "implement this specific function" can't decide to refactor the whole module.

**Rule 3: No Shared State**

In tool-heavy environments, multi-agent systems paid massive coordination taxes. Tools become shared state. Multiple agents accessing same resources create contention. Tool selection accuracy degrades as catalogs grow beyond a few dozen options.

**The rule:** Workers operate in complete isolation. No shared state, no communication, no awareness of each other. Keep tool sets small — 3-5 core tools always available, others on demand through progressive disclosure. Coordination through external mechanisms designed for concurrent access (git for code, task queues for assignment, test suites for validation). Isolated workers need dedicated merge infrastructure (Gas Town's Refinery).

**Rule 4: Plan for Endings**

Context accumulation creates a serial dependency with the agent's own past. Signal dilutes in noise ("context pollution"). The "lost in the middle" phenomenon persists even with massive context windows. Cursor found drift unavoidable during continuous operation — quality degraded within hours.

**The rule:** Episodic operation with planned resets. Each cycle runs for a bounded period, captures results to external storage, terminates. Next cycle starts fresh. External memory provides reference points immune to drift. Treat endings as natural boundaries to design around, not failures to prevent. (Gas Town: sessions as ephemeral cattle, work as externally-stored "molecules," GUPP principle, Nondeterministic Idempotence.)

**Rule 5: Prompts Over Infrastructure**

Cursor found "a surprising amount of behavior comes down to how we prompt the agents." Sophisticated coordination infrastructure often adds serial dependencies rather than removing them. Good prompts reduce coordination needs — an agent that clearly understands its role, boundaries, and success criteria doesn't need to check with other agents.
...
Tests serve a deeper architectural function for multi-agent systems: they provide coordination without creating dependencies between agents. Multiple workers can consult the test suite independently without creating any dependencies between them. Tests coordinate without serializing.

**The rule:** Design work around domains with testable correctness. Test suites become an architectural element enabling parallel work by providing shared ground truth without shared state. If you don't have test suites, you need shorter feedback loops and more human oversight.

### 5.8 Where Complexity Belongs

**The Inversion:** Complexity can live in agents or in the orchestration layer. These have very different scaling properties.

| Complexity Location | Effect | Scaling Property |
|--------------------|---------|--------------------|
| **In agents** | Agent coordinates with others, maintains shared state, understands big picture | Creates serial dependencies. Adding more agents adds more entanglement. System becomes harder to parallelize. |
| **In orchestration** | External systems feed work, monitor progress, merge outputs, track state | Enables parallelism. Simple agents need external systems, but those systems don't create agent-to-agent dependencies. |

**The Kubernetes Analogy:** "Both systems coordinate unreliable workers toward a goal." Control plane watches execution nodes, local agents monitor ephemeral workers. "Kubernetes asks 'Is it running?' while Gas Town asks 'Is it done?'" Workers are cattle either way. Intelligence lives in the systems that herd them.

**Design Implication:** Investment goes into orchestration, not agent sophistication. Build systems that can feed, monitor, and merge the outputs of many simple workers. Don't build elaborate agents that need to understand the world to function.

### 5.9 Multi-Agent Scaling Architecture Summary
...
| Small tool sets | 3-5 core tools, others on demand. |
| Tests as coordination | Shared ground truth without shared state. |

**Anti-Pattern Detection:** If you hear "agents collaborating like teams," "sharing context dynamically," or "rich inter-agent communication" — you're looking at architecture that works in demos but breaks in production.

### 5.10 Multi-Agent Scaling Diagnostic Protocol

An 8-step diagnostic sequence for designing or evaluating multi-agent architectures. Full prompts preserved in `MULTI-AGENT-SCALING-REFERENCE.md` Parts 2-3.

| Step | Diagnostic | Micro-Skill | What It Produces |
|------|-----------|------------|-----------------|
| 1 | Scale or Fix First? | 5.1-A | Pattern match + scale decision + first bottleneck + concrete next step |
| 2 | Coordination Choke Points | 5.2-A | Shared resources inventory + wait points + top 3 choke points + fix recommendation |
| 3 | Two-Tier Architecture Templates | 5.3-A | Planner / Worker / Judge system prompt templates |
| 4 | Worker Boundaries | 5.4-A | Max task size + allowed actions whitelist + forbidden actions blacklist |
| 5 | Tool Diet | 5.5-A | Always-on tools (3-5) + on-demand tools + tools to remove |
| 6 | Session Lifecycle | 5.6-A | Timebox + checkpoint protocol + restart handoff |
| 7 | Judge Criteria | 5.7-A | Pass/fail gates + graded criteria + decision rules |
| 8 | Verification + Merge Policy | 5.8-A | Verification gates + merge authority + conflict resolution |

**Execution:** Run in sequence — each step's output feeds the next. The complete sequence produces a full multi-agent architecture document.

**Two delivery modes:**
1. **Multi-Step Kit** — 8 separate prompts, run in order (allows thinking between steps)
2. **Single-Session Guide** — One guided conversation, 30-60 minutes (one sitting)
