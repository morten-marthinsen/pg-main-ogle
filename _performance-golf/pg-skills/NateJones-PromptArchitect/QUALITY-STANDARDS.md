...
### 1.5 Multi-Agent Architecture Quality (4 dimensions — apply only to multi-agent systems)

| Dimension | What It Measures | Scale | Threshold | Critical? |
|-----------|-----------------|-------|-----------|-----------|
| Serial Dependency Count | Number of points where one agent's work blocks another's | 0-∞ (lower=better) | 0 (production) / ≤2 (prototype) | Yes |
| Tier Compliance | Adherence to two-tier hierarchy (Planner → Worker → Judge) | 0-5 | ≥4 | Yes |
| Worker Isolation | Degree of isolation between workers (no shared state, no communication, no mutual awareness) | 0-5 | ≥4 | Yes |
| Orchestration Complexity Ratio | Complexity in orchestration layer ÷ total system complexity (higher = complexity in the right place) | 0.0-1.0 | ≥0.70 | No |
...
### 2.6 Multi-Agent Architecture Scoring

**Serial Dependency Detection:**

```
1. Inventory all coordination points:
   - Shared files/databases accessed by multiple agents
   - Message queues where agents wait for each other
   - Locks, semaphores, or turn-taking mechanisms
   - Shared context or memory stores
   - Tool contention (multiple agents using same external tool)

2. Count SERIAL DEPENDENCIES:
   - Any point where Agent B cannot proceed until Agent A completes
   - Any shared state that requires coordination to access
   - Any communication channel between peer-level agents
   - Any tool that serializes access across agents

3. Score:
   0 dependencies = Optimal (fully parallel)
   1-2 dependencies = Acceptable (prototype/development)
   3-5 dependencies = Weak (scaling will degrade performance)
   6+ dependencies = Failing (multi-agent overhead exceeds benefit)
```

**Tier Compliance Scoring (0-5):**

| Score | Criteria |
|-------|----------|
| 0 | No tier structure — flat agent collaboration |
| 1 | Informal hierarchy but agents still coordinate laterally |
| 2 | Two tiers exist but workers share state or communicate |
| 3 | Two tiers with mostly isolated workers, some leakage |
| 4 | Strict two-tier with isolated workers, clean Planner/Worker/Judge separation |
| 5 | Two-tier with full isolation, workers ignorant of each other, complexity entirely in orchestration layer |

**Worker Isolation Scoring (0-5):**

| Score | Criteria |
|-------|----------|
| 0 | Workers share state, communicate, and coordinate freely |
| 1 | Workers share state but don't communicate directly |
| 2 | Workers don't share state but have awareness of other workers |
| 3 | Workers isolated but receive more context than minimum viable |
| 4 | Workers receive minimum viable context, operate in isolation, no shared state |
| 5 | Full structural isolation: workers cannot access shared state even if they wanted to (information hiding enforced by architecture, not instruction) |

**Orchestration Complexity Ratio:**

```
1. Classify each system component:
   - ORCHESTRATION: Task queues, planners, judges, merge systems,
     state stores, monitoring, checkpoint infrastructure
   - AGENT: Worker prompt complexity, worker decision logic,
     worker context awareness, worker coordination behavior

2. Calculate:
   ratio = orchestration_complexity / (orchestration_complexity + agent_complexity)

3. Score:
   ≥0.80 = Optimal (dumb agents, smart orchestration)
   0.70-0.79 = Good (complexity mostly in right place)
   0.50-0.69 = Weak (complexity split — unclear boundaries)
   <0.50 = Failing (smart agents, dumb orchestration — will not scale)
```

**Multi-Agent Anti-Pattern Checklist:**

Each anti-pattern present = 1 violation. Threshold: 0 violations for production.

| # | Anti-Pattern | Detection Signal |
|---|-------------|-----------------|
| 1 | Flat team collaboration | Agents described as "collaborating," "debating," or "reaching consensus" |
| 2 | Shared dynamic context | Agents reading/writing shared memory, context, or state stores |
| 3 | Rich inter-agent communication | Agents sending messages to each other (not through orchestrator) |
| 4 | Continuous operation | Agents running indefinitely without planned resets |
| 5 | Deep hierarchy (3+ levels) | More than Planner → Worker → Judge |
| 6 | Large tool sets | Workers with >5 core tools always available |
| 7 | Workers understand big picture | Worker prompts include system-level context or goals |
| 8 | Coordination infrastructure over prompts | Sophisticated inter-agent protocols instead of clear prompt specs |
...
### Multi-Agent Orchestrator Skills
- Serial Dependency Count: REQUIRED (0 for production)
- Tier Compliance: REQUIRED (≥4)
- Worker Isolation: REQUIRED (≥4)
- Orchestration Complexity Ratio: ≥0.70
- Anti-Pattern Checklist: REQUIRED (0 violations)
- Session Lifecycle: REQUIRED (episodic operation with planned resets)
- Must have: Planner/Worker/Judge templates, worker boundary definitions, tool diet specs, merge policy
- Must NOT have: flat collaboration, shared context, continuous operation, deep hierarchy, agent-to-agent communication
...
quality_scores:
  target: [filename]
  type: [orchestrator | leaf | generation | validation | scraper | multi_agent_orchestrator]
  timestamp: [ISO date]
  ...
  multi_agent: # Only for multi-agent orchestrator systems
    serial_dependencies:
      score: [count]
      threshold: 0
      status: [PASS | FAIL | N/A]
      dependencies_found: [list of coordination points]
    tier_compliance:
      score: [0-5]
      threshold: 4
      status: [PASS | FAIL | N/A]
      structure: [description of tier hierarchy]
    worker_isolation:
      score: [0-5]
      threshold: 4
      status: [PASS | FAIL | N/A]
      isolation_gaps: [list of leakage points]
    orchestration_ratio:
      score: [0.0-1.0]
      threshold: 0.70
      status: [PASS | FAIL | N/A]
      orchestration_components: [list]
      agent_components: [list]
    anti_pattern_violations:
      score: [count, lower=better]
      threshold: 0
      status: [PASS | FAIL | N/A]
...
**Scope:** All Claude Code skill files and multi-agent architectures within Anthony's vault ecosystem
**Location:** `/Users/anthonyflores/Desktop/Manual Library/Anthony-Main-Vault/NateJones-PromptArchitect/QUALITY-STANDARDS.md`

**v1.1 Changes:**
- Added §1.5 Multi-Agent Architecture Quality (4 dimensions)
- Added §2.6 Multi-Agent Architecture Scoring (serial dependency detection, tier compliance, worker isolation, orchestration ratio, anti-pattern checklist)
- Added Multi-Agent Orchestrator skill type modifier (§4)
- Added `multi_agent_orchestrator` type to scoring report format (§5)
- Added multi-agent scoring dimensions to report YAML schema
